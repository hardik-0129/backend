  ...
  try {
    ...
    if (user.referredBy && !user.referralFirstPaidCredited && slot.entryFee > 0) {
      ...
      if (referrer && String(referrer._id) !== String(user._id)) {
        ...
        console.log(`Referral bonus processed: ${referrer.name} received 5 coins for ${user.name}'s first paid match`);
      }
    }
  } catch (error) {
...
//   // Validate total amount calculation (flat keys: Team1-B: true)
//   const positionsCount = Object.keys(selectedPositions).length;
...
//     // Validate that all selected positions have player names (flat keys)
//     const missingNames = [];
